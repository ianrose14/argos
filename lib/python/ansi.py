...
def getcode(code):
    return "%s[%dm" % (chr(27), ANSI_CODES[code])

def format(text, codes=[], fd=sys.stdout, reset=True, force=False):
    if not force and fd is not None:
...
if __name__ == '__main__':
    ...
    write(" test\n")
    print "here is a", format("dim green", ['dim', 'green']), "and", format("dim blue", ['dim', 'blue'])
    ...
    print "here I", getcode("underline"), "start underline", getcode("underline_off"), "and stop it"
    print "here I", getcode("bold"), "start bold", getcode("underline_off"), "and stop underline", getcode("reset")
    print "start", getcode("red"), "red, then", getcode("bold"), "add bold, then", getcode("reset color"), "reset color only"
    print getcode("reset"),
    print "the end!"
